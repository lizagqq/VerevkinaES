#!/usr/bin/env python3
"""
Параллельная версия с одномерной декомпозицией (по x)
"""
...
def solve_2d_1d_decomposition():
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    size = comm.Get_size()
    
    # Создание линейной топологии
    ...
    rcounts_N_x, displs_N_x = auxiliary_arrays_determination(N_x+1, size)
    N_x_part = rcounts_N_x[rank_cart]
    
    # С граничными элементами
    if rank_cart in [0, size-1]:
        N_x_part_aux = N_x_part + 1
    else:
        N_x_part_aux = N_x_part + 2
    
    displ_x = displs_N_x[rank_cart]
    ...
    for i in range(N_x_part_aux):
        ...
        for j in range(N_y+1):
            u_part_aux[0, i, j] = u_init(x[global_i], y[j], eps)
    ...
    for m in range(M+1):
        ...
        if rank_cart == size - 1:
            for j in range(N_y+1):
                u_part_aux[m, -1, j] = u_right(y[j], t[m])
        for i in range(N_x_part_aux):
    ...
    if rank == 0:
        ...
        print(f"1D декомпозиция на {size} процессах: {elapsed:.4f} сек")
    
    comm_cart.Free()

if __name__ == "__main__":
    solve_2d_1d_decomposition()
